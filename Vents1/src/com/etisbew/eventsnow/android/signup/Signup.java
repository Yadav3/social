	...
	public void onClick(View v) {
		...
		if (v.getId() == R.id.update) {
			if (email.getText().toString().length() > 0
					...
					&& ccity.getText().toString().length() > 0
					&& cstate.getText().toString().length() > 0
				...
				if (pwd.getText().toString().equals(cpwd.getText().toString())) {
					update_profile = util.setNewMyAccount(fname.getText()
							.toString(), email.getText().toString(), pwd
							.getText().toString(), cpwd.getText().toString(),
							cname.getText().toString(), phone.getText()
				...
				util.dialogExample1("All Fields are Mandatory Except Company Name");
			}
		...
		if (select_flag == 1) {
			...
			for (int i = 0; i < state_name.size(); i++) {
				...
				if (str.contains(cstate.getText().toString())) {
					...
					/*
					 * if(given_state.equalsIgnoreCase(
					 * cstate.getText().toString())){
					 * 
					 * }else{
					 */
