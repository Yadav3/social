...
public class GalleryImageAdapter2 extends BaseAdapter {
	...
	public GalleryImageAdapter2(Activity context, int resource,
			ArrayList<String> plotsImages) {
		
		this.context = context;
		this.plotsImages = plotsImages; 
		this.row = resource;
		System.out.println("plotsimages"+plotsImages+plotsImages.size());
	}
...
//position=position-1;
		 ...
		 * 
		 * // imageView = new ImageView(this.context); txt = new
		 * TextView(this.context);
		 *  
		 * // imageView.setPadding(3, 3, 3, 3); // // convertView = imageView;
		 * convertView = txt;
		 * 
		 * // holder.imageView = imageView; holder.textview = txt;
		 * 
		...
		try {
			...
			System.out.println("ss"+plotsImages.size()+position);
			String ss=plotsImages.get(position);
