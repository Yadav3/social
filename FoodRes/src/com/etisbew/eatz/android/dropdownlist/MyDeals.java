...
public class MyDeals extends BaseActivity {
...
//				MyDeals.this.finish();
		...
		lvOrders.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int pos,
				...
				try {
					Appconstants.strWebviewUrl = jsonArray.getJSONObject(pos)
							.getString("voucher_link").toString();
				} catch (JSONException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} 
				//Appconstants.strWebviewUrl ="http://dev.eatz.com/files/order_invoice_3083.pdf";
				/*Intent intent = new Intent(MyDeals.this, PdfActivity.class);
				intent.putExtra(PdfViewerActivity.EXTRA_PDFFILENAME, Appconstants.strWebviewUrl);
		...
		if (ConnectivityReceiver.checkInternetConnection(MyDeals.this)) {

			new MyTask().execute();

		} else {
			ConnectivityReceiver.showCustomDialog(MyDeals.this);
		}
...
//		new MyTask().execute();
	...
	class MyTask extends AsyncTask<String, Void, String> {
		...
		protected void onPreExecute() {
			...
			pDialog.setMessage("Loading your deals. Please wait...");
		...
		protected String doInBackground(String... params) {
			String url=Appconstants.url2 + "myDeals/"
		...
		protected void onPostExecute(String result) {
			...
			if (null == result || result.length() == 0) {
				MyDeals.this.finish();
				showDialogMsg(MyDeals.this, "No data found from web!!!");

			} else {

				try {
					JSONObject mainJson = new JSONObject(result);
					jsonArray = mainJson.getJSONArray("Deals");
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						objItem.setItemName(objJson.getString("title"));
						objItem.setVouchers(objJson.getString("vouchers"));
						objItem.setVoucherType(objJson
								.getString("voucher_type"));
						objItem.setUnitPrice(objJson.getString("unit_price"));
						objItem.setQuantity(objJson.getString("quantity"));
						objItem.setPrice(objJson.getString("total"));
					...
					}if (mainJson.has("points_pending")) {
	...
	public void setAdapterToListview() {
		objAdapter = new RowAdapter(MyDeals.this, R.layout.mydeals1,
				arrayOfList);
		...
		protected void onPostExecute(String result) {
			...
			if (null == result || result.length() == 0) {
				...
				try {
					...
					showDialogMsg(MyDeals.this, strErrorMsg);
	...
	public void RateVenue() {
		...
		dialog1 = new Dialog(MyDeals.this);
		...
		btnWriteReviewSubmit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (TextUtils.isEmpty(edtWRTitle.getText().toString())) {
					showDialogMsg(MyDeals.this, "Please enter review title");
				}
				if (TextUtils.isEmpty(edtReviewComments.getText().toString())) {
					showDialogMsg(MyDeals.this, "Please enter review comments");
				}
				if (!TextUtils.isEmpty(edtWRTitle.getText().toString())
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionId == Explore.ID_LOGOUT) {
							...
							startActivity(new Intent(MyDeals.this, Login.class)
									.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
							finish();
						} else if (actionId == Explore.ID_MYACCOUNT) {
							if (ConnectivityReceiver.checkInternetConnection(MyDeals.this)) {

								new AccountDetails(MyDeals.this).execute();
...
//							new AccountDetails(MyDeals.this).execute();
							...
							startActivityForResult(new Intent(MyDeals.this,
